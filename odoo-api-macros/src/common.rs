...
use syn::{ItemStruct, Fields, Lit, AttributeArgs, NestedMeta, Meta, MetaNameValue, FieldsNamed};
...
impl syn::parse::Parse for ItemStructNamed {
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        ...
        match &item {
            Ok(ItemStruct { fields: Fields::Named(fields), .. }) => {
                let item = item.clone().unwrap();
                let mut doc_head = String::new();
                for attr in &item.attrs {
                    if let Meta::NameValue(MetaNameValue { path, lit: Lit::Str(lit), .. }) = attr.parse_meta()? {
