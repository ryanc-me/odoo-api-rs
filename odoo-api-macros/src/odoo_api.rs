...
impl TryFrom<AttributeArgs> for OdooApiArgs {
    ...
    fn try_from(value: AttributeArgs) -> Result<Self> {
        ...
        Ok(Self {
            service: service.ok_or("The \"service\" key is required (e.g., `service = \"object\"`)")?,
            method: method.ok_or("The \"method\" key is required (e.g., `method = \"execute\"`)")?,
            auth,
            name,
        })
    }
...
pub(crate) fn odoo_api(args: AttributeArgs, input: ItemStructNamed) -> Result<TokenStream2> {
    ...
    let name_call = if let Some(name) = &args.name { name.clone() } else { args.method.clone() };
...
fn impl_client(ident_struct: &Ident, ident_call: &Ident, args: &OdooApiArgs, fields: &FieldsNamed, doc: &str) -> Result<TokenStream2> {
